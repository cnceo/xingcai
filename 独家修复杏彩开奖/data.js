...
mysqlConnection=createMySQLClient();
//{{{
...
function submitData(data, conf){
	...
	//check same open data
	check_same_codes(data.type, data.number, data.data, function () {
		
		mysqlConnection.query("insert into blast_data(type, time, number, data) values(?,?,?,?)", [data.type, data.time, data.number, data.data], function(err, result){
			//console.log(err);
			if(err){
				//console.log(err);
				// 普通出错
...
function check_same_codes(type, action_no, open_number, callback) {
    ...
    var last_no = parseInt(action_no.substr(action_no.length - 3, 3)) - 1;
    last_no = action_no.substr(0, action_no.length - 3) + '' + last_no;
	console.log(last_no);
	
	var mySql="SELECT * FROM blast_data WHERE type="+type+" AND number='"+last_no+"' AND data='"+open_number+"'";
	
    mysqlConnection.query(mySql, function (err, result) {
        if (result.length > 0) {
            //撤单
            console.error("存在连续重复开奖号码，统一撤单");
			var callSql="call same_cancel_bets("+type+",'"+action_no+"')";
			console.log(callSql);
            mysqlConnection.query("call same_cancel_bets("+type+",'"+action_no+"')", function (err, res) {
				
                if (err) {
                    //logger.error("撤单失败：", err);
				    console.log("err faile",err);
                } else {
                    //logger.warn(`撤单成功信息：彩种=${type},期数=${action_no}`);
					console.log("back success,Action:",action_no);
                    callback();
                }
            });
        } else {
...
http.createServer(function(req, res){
    ...
    data = querystring.parse(reqArr[1]);
		...
		delete data.key;
			...
			if(reqArr[0]=='/data/add'){
				submitDataInput(data);
			}else if(reqArr[0]=='/data/kj'){
				console.log(data);
				calcJ(data, true)
			}

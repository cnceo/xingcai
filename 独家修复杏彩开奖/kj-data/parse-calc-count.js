...
exports.ssclhwq=function(bet, kj){
	return ssclh(bet, kj, 1, 2);
     }

exports.ssclhwb=function(bet, kj){
	return ssclh(bet, kj, 1, 3);
     }
	 exports.ssclhws=function(bet, kj){
	return ssclh(bet, kj, 1, 4);
     }
	 exports.ssclhwg=function(bet, kj){
	return ssclh(bet, kj, 1, 5);
     }
	 exports.ssclhqb=function(bet, kj){
	return ssclh(bet, kj, 2, 3);
     }
	 exports.ssclhqs=function(bet, kj){
	return ssclh(bet, kj, 2, 4);
     }
	 exports.ssclhqg=function(bet, kj){
	return ssclh(bet, kj, 2, 5);
     }
	 exports.ssclhbs=function(bet, kj){
	return ssclh(bet, kj, 3, 4);
     }
	 exports.ssclhbg=function(bet, kj){
	return ssclh(bet, kj, 3, 5);
     }
	 exports.ssclhsg=function(bet, kj){
	return ssclh(bet, kj, 4, 5);
     }
	 //}}}
...
// 时时彩龙虎和中奖函数 qq158115595
//龙 
exports.ssc_lhh_l=function(betData, kjData){
kjData=kjData.split(',');betData=betData.split(' ');
var qq158115595 = 0;
for(var i=0;i<betData.length;i++){
	if(betData[i] =='万千' && kjData[0]>kjData[1])  qq158115595+=1;
	if(betData[i] =='万百' && kjData[0]>kjData[2])  qq158115595+=1;
	if(betData[i] =='万十' && kjData[0]>kjData[3])  qq158115595+=1;
	if(betData[i] =='万个' && kjData[0]>kjData[4])  qq158115595+=1;
	if(betData[i] =='千百' && kjData[1]>kjData[2])  qq158115595+=1;
	if(betData[i] =='千十' && kjData[1]>kjData[3])  qq158115595+=1;
	if(betData[i] =='千个' && kjData[1]>kjData[4])  qq158115595+=1;
	if(betData[i] =='百十' && kjData[2]>kjData[3])  qq158115595+=1;
	if(betData[i] =='百个' && kjData[2]>kjData[4])  qq158115595+=1;
	if(betData[i] =='十个' && kjData[3]>kjData[4])  qq158115595+=1;
 }
 return qq158115595;
...
for(var i=0;i<betData.length;i++){
	if(betData[i] =='单' && sum%2!=0)  qq158115595+=1;
	if(betData[i] =='双' && sum%2==0)  qq158115595+=1;
	if(betData[i] =='大' && sum<10 && sum>4)  qq158115595+=1;
	if(betData[i] =='小' && sum<10 && sum<5)  qq158115595+=1;
 	if(betData[i] =='大' && sum>9 && sum%10>4)  qq158115595+=1;
	if(betData[i] =='小' && sum>9 && sum%10<5)  qq158115595+=1; 
 }
	return qq158115595;
}
//// 牛牛玩法中奖函数 qq158115595
...
//// 牛牛大小单双玩法中奖函数 qq158115595
exports.niuniudxds=function(betData, kjData){
	var qq158115595 = 0;betData=betData.split('');kj=kjData.split(',');
	var cn = ['牛','一','二','三','四','五','六','七','八','九','牛'];
	if((parseInt(kj[0],10)+parseInt(kj[1],10)+parseInt(kj[2],10))%10 ==0 ){
		var a =parseInt(kj[3])+parseInt(kj[4]);if(a>9) a=a-10;
		if(a%2==0 && betData.indexOf('双')!=-1) qq158115595+=1;
		if(a%2!=0 && betData.indexOf('单')!=-1) qq158115595+=1;
		if(a>4 && betData.indexOf('大')!=-1) qq158115595+=1;
		if(a<5 && betData.indexOf('小')!=-1) qq158115595+=1;
	 }
	if((parseInt(kj[0],10)+parseInt(kj[1],10)+parseInt(kj[3],10))%10 ==0 ){
...
//时时彩龙虎
function ssclh(bet, kj, num1, num2){
	kj=kj.split(',');
	val1=parseInt(kj[num1-1],10);
	val2=parseInt(kj[num2-1],10);
	bet=bet.split('');
	count=0;
	for (var i=0,l=bet.length; i<l; i++){
		if(bet[i]=='龙'){
